...
From the explanation above we can also conclude that the way this model interprets and learns the data is rather different
from what KNN does: Whereas KNN classifies each sample by looking at its (usually small) set of nearest neighbors in
the training set, SVM models the common traits of each class digit in its weights vector ($\vec{w_j}$) using all of the
samples, and then uses these weights directly to classify new samples without needing the original samples again.
