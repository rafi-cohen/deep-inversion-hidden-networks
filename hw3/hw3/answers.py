...
By looking at the results it seems that the samples generated by the VAE are significantly more blurry than those 
generated by the GAN.
According to the [2017 paper - Adversarially Learned Inference](https://openreview.net/pdf?id=B1ElR4cgg), this is a
known problem with VAEs: Since these type of networks combine the maximum likelihood training paradigm with a
conditional independence assumption on the output given the latent variables - they tend to distribute probability mass
diffusely over the data space. The direct consequence of this is that image samples from VAE-trained models tend to be
blurry. Intuitively, given some $\bb{z}$ sampled from the latent space, if $q(X|z)$ is spread diffusely over the data
space, the instance $\Psi(\bb{z})$ we would get would depend on a larger part of the instance space, and thus would be
blurrier. On the other hand, since GANs are not based on the above paradigm, they do not suffer from this issue.
